...
class _ScenarioEditorServicesCardViewState
    ...
    if (widget.item.children.isNotEmpty) {
      _currentFunctions.addAll(
        widget.item.children.map(
          (block) {
            ...
            return originFunction.copyWith(
              arguments: argumentsWithValue,
            );
          },
        ),
      );
      ...
      if(widget.scheduledThings != null) {
        for(var scheduledThing in widget.scheduledThings!) {
          /// "_(#thing1).func_no_arg" -> "func_no_arg"
          // _thingFunctionListForTag.add((Tag(name: scheduledThing.service.split('.')[1]), 'assets/icons/tmp_light_on.svg'));
          _thingFunctionListForTag.addAll(widget.getServiceFunctionListByTag(widget.initialSelectedTag!));
        }
        // 중복 제거
        _thingFunctionListForTag = _thingFunctionListForTag.toSet().toList();
      }
      */

      /*
      _argumentIsSetList.addAll(
        _currentBlocks.map(
                (e) {
              if(e is FunctionServiceBlock) {
                return e.arguments.map((e) => e.value != null && e.valueString.isNotEmpty).toList();
              } else {
                return [];
              }
            }
      ...
      _detailOptionSettingExpansionTileOpened = _argumentIsSetList[0].isNotEmpty;
      _argumentNameList.addAll(
        _currentBlocks.map((e) {
          if(e is FunctionServiceBlock) {
            return e.arguments.map((e) => TextEditingController(text: e.valueString)).toList();
          } else {
            return [];
          }
        }),
  ...
  Future setData() async {
    ...
    widget.onChanged(FunctionServiceListBlock(changedChildren));
  }
}
